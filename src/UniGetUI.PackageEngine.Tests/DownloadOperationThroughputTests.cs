...
/// <summary>
/// Proves the HTTP <see cref="DownloadOperation"/> path receives a measured
/// download speed through the shared generic operation layer: a throttled
/// loopback server streams a real payload, and at least one progress report
/// must carry a finite positive <c>BytesPerSecond</c>. No synthetic speeds.
/// </summary>
public sealed class DownloadOperationThroughputTests
{
    ...
    /// <summary>
    /// Minimal throttled HTTP/1.1 server over loopback TCP: serves one fixed
    /// payload with <c>Content-Length</c>, pacing chunks so the client's real
    /// clock observes distinct progress samples.
    /// </summary>
    private sealed class ThrottledLoopbackServer : IDisposable
    {
        ...
        public ThrottledLoopbackServer(int totalBytes, int chunkSize, TimeSpan chunkDelay)
        {
            ...
            _listener.Start();
            Url = new Uri($"http://127.0.0.1:{((IPEndPoint)_listener.LocalEndpoint).Port}/payload.bin");
        ...
        private async Task ServeOnceAsync()
        {
            ...
            while (seen < request.Length - 1)
            {
                ...
                seen += read;
                if (
                    Encoding.ASCII.GetString(request, 0, seen).Contains("\r\n\r\n")
                )
                    break;
            }
    ...
    public async Task HttpDownload_ReportsMeasuredThroughput()
    {
        ...
        try
        {
            ...
            Assert.Contains("MB/s", OperationProgressFormatter.Format(last));
        }
        finally
        {
